...
class MusicCog(Cog):
    ...
    async def on_song_end(vc: wavelink.Player):
        pass
    ...
    async def play(self, ctx: ApplicationContext, url: str):
        ...
        if len(vc.channel.members) == 0:
            await vc.disconnect()
        song = await wavelink.YouTubeTrack.search(url)
        # global song_queue
        ...
        # song_queue= {ctx.guild_id: song}

        # song_queue: dict
        # if not len(song_queue[ctx.guild_id]) == 0:
        # if song_queue[ctx.guild_id] != '':
